...
class Handler
{
    ...
    protected function saveTempContent($path = null, $ext = "tmp")
    {
        ...
        if (! isset($path)) $path = $this->getTempFolder() . "/";
        $source = $path . Utility::getUniqueId().".$ext";
        ...
        $content = preg_replace_callback('~<script ([^>]*) src=["\']([^>]*)["\']~', 
            function($matches) use ($http_host, $url) {
                $href = $matches[2];
                $isAbsolutePath = substr($href, 0, 1) === "/" || 
                    substr($href, 0, 4) === "http";
    ...
    {
        $nodeBinary = Utility::get($params, 'nodeBinary', null);
        $chromeBinary = Utility::get($params, 'chromeBinary', null);
        // $pdf = Utility::get($params, 'pdf', []);
        ...
        if (is_string($nodeBinary)) {
            ...
            $url = $this->getFullUrl();
            $url = substr($url, 0, strrpos($url, "/"));
            $tmpPDFFile = Utility::getUniqueId().".pdf";
        ...
        else if (is_string($chromeBinary)) {
            $tmpFolder = $this->getTempFolder();
            $tmpHtmlFile = $this->saveTempContent("", "html");
            $url = $this->getFullUrl();
            $url = substr($url, 0, strrpos($url, "/"));
