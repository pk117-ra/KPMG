...
// echo $where;
 $join = 'FROM `category` as a LEFT JOIN user_master as b ON a.CreatedBy = b.UserId AND a.AppCode = b.AppCode';
  // Page Connection //
 // category_update.php//
$columns = array(
		array('db' => 'a.CategoryId' , "dt" => 0 ,'field' => 'CategoryId'),
		array('db' => 'a.CategoryId' , "dt" => 1 ,'field' => 'CategoryId' , 'formatter' => function($d,$r)
			{
				if(idget() == 1)
				{
					return "<a href='product_group_update.php?cateId=".base64_encode(base64_encode(base64_encode(base64_encode($d))))."' style='font-size: 25px; color: #6f42c1;' data-toggle='tooltip' title='Edit'><i class= 'fa fa-edit'></i></a>&nbsp;&nbsp;<i class='fa fa-trash deleteid' tname='category' acode ='".appcodeget()."' deleteid = '".$d."' colname = 'CategoryId' aria-hidden='true' style='font-size: 25px;cursor:pointer; color: red' data-toggle='tooltip' title='Delete' data-toggle='tooltip' title='Delete'></i>";
				}
				elseif(idget() == 2)
				{
					return "&nbsp;&nbsp;<i class='fa fa-undo restoreid' tname='category' acode ='".appcodeget()."' restoreid = '".$d."' colname = 'CategoryId' aria-hidden='true' style='font-size: 23px;cursor:pointer; color: red;' data-toggle='tooltip' title='Restore'></i>";
				}
			}),
		array('db' => 'a.ProductCategoryName' , "dt" => 2 ,'field' => 'ProductCategoryName'),
